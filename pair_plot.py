...
def pair_plot(data):
    ...
    courses = [f for f in list(data.df.columns) if np.issubdtype(
        data.df[f].dtype, np.number)][1:]
    n = 0
        ...
        for index_y, course_y in enumerate(courses):
            ...
            if course_x == course_y:
                for house, color in zip(data.houses, data.colors):
                    marks = data.df.loc[data.df['Hogwarts House']
                                        == house][course_x]
                ...
                for house, color in zip(data.houses, data.colors):
                    x = data.df.loc[data.df['Hogwarts House']
                                    == house][course_x]
                    y = data.df.loc[data.df['Hogwarts House']
                                    == house][course_y]
